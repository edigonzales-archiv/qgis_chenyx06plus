...
class TestTriangleOverlap( QObject ):
    ...
    def run( self, iface ):
        ...
        query = "BEGIN; "
        query += "DELETE FROM " + self.schema + ".triangle_overlap_lv03; "
        query += "INSERT INTO " + self.schema + ".triangle_overlap_lv03 (error_type, the_geom) SELECT 'overlap' as error_type, ST_Multi(the_geom) as the_geom FROM ( SELECT a.ogc_fid, ST_Intersection(a.the_geom, b.the_geom) as the_geom FROM "+self.schema+".dreiecke_lv03_bearbeitung as a, "+self.schema+".dreiecke_lv03_bearbeitung as b WHERE a.the_geom && b.the_geom AND ST_Overlaps(a.the_geom, b.the_geom) AND NOT ST_Equals(a.the_geom, b.the_geom)) as a WHERE geometrytype(the_geom) IN ('POLYGON', 'MULTIPOLYGON'); "
        query += "COMMIT; "
        print query
        ...
        table = {}
        table["type"] = "pg"        
        table["title"] = u"Dreiecksüberlappungen"
        table["schema"] = self.schema
        table["table"] = "triangle_overlap_lv03"
        table["geom"] = "the_geom"
        table["key"] = "ogc_fid"            
        table["sql"] = ""
        table["group"] = ""
        table["style"] = "dreiecke/overlaps.qml"
        vlayerTriangleOverlaps = utils.doShowSimpleLayer( iface, table, False, True )
